...
const Home: React.FC = () => {
    ...
    if (loading) return <div>Đang tải danh sách project...</div>;
    if (error) return <div className="text-red-500">{error}</div>;

    // Hàm để tô màu Priority
    ...
    // Hàm để tô màu Checklist Status
    const getChecklistColor = (status: string) => {
        switch (status) {
            ...
            case 'In Progress': return 'text-blue-700 bg-blue-100 dark:text-blue-300 dark:bg-blue-900/30';
            case 'Not Started': return 'text-gray-700 bg-gray-100 dark:text-gray-300 dark:bg-gray-700/30';
            default: return 'text-gray-700 bg-gray-100 dark:text-gray-300 dark:bg-gray-700/30';
        }
    };
            ...
            <main className="pt-20 px-6 pb-12">
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 mb-8">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                        <button className="flex items-center gap-2 px-5 py-3 text-sm font-medium text-white bg-green-500 rounded-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 transition-shadow">
                            <Plus className="h-5 w-5" />
                            Add New Project
                        </button>
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full sm:w-auto">
                            <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg">
                                <button className="px-6 py-2.5 text-sm font-semibold text-white bg-green-500 rounded-lg">
                                    All
                                </button>
                            </div>
                                ...
                                {projects.map((project) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 flex gap-2">
                                            <button
                                                className="p-1 rounded-md bg-green-500 hover:bg-green-600 text-white transition-colors"
                                                title="Copy"
                                            >
                                        ...
                                        {/* Priority */}
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${getPriorityColor(project.priority)}`}>
                                                {project.priority}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${getChecklistColor(project.checklistStatus)}`}>
                                                {project.checklistStatus}
                                            </span>
                                        </td>
